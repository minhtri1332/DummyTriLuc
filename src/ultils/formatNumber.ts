...
export const formatFloat = (value: string | number | undefined) => {
  try {
    if (typeof value === undefined) {
      return "0";
    }
    const dataValueSplit = String(value).split(".");
    if (dataValueSplit.length == 1) {
      return String(value).replace(moneyRegex, ",");
    } else {
      return (
        String(dataValueSplit[0]).replace(moneyRegex, ",") +
        "." +
        dataValueSplit[1]
      );
    }
...
export const formatNumberShortCompact = (
...
export const formatMoneyDynamic = (
  ...
  try {
    ...
    const absNum = Math.abs(num);
    if (absNum >= max) {
      return formatNumberShortCompact(value);
    }
    return formatMoney(value);
  } catch (error) {
    return formatNumberShortCompact(value);
...
export const getRelativeYears = (from: moment.Moment, to: moment.Moment) => {
  const years = to.diff(from, "years");
  const sameYearWithTo = from.clone().add(years, "years");
  const days = to.diff(sameYearWithTo, "days");
  return {
    years,
    days,
  };
